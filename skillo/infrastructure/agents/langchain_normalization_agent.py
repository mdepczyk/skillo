...
from skillo.infrastructure.schemas import (
...
class LangChainNormalizationAgent:
    ...
    def normalize_cv_data(
        self, cv_response: DocumentProcessingResponse, profile: str
    ) -> NormalizationResponse:
...
def create_normalization_agent(config: Config) -> LangChainNormalizationAgent:
    return LangChainNormalizationAgent(config)
